...
def parse_behaviors(source, target, user2int_path):
    ...
    if config.analysis_news_popular:
        news_freq = {}
        for news_list in behaviors['clicked_news']:
        # Split the string into individual news IDs
            news_items = news_list.split()
            # Count each news item
            for news in news_items:
                if news in news_freq:
                    news_freq[news] += 1
                else:
                    news_freq[news] = 1
        save_news_freq(config.original_data_path + '/news_freq.pkl', news_freq)
        print("news freq are saved to " + config.original_data_path + "/news_freq.pkl")
...
if __name__ == '__main__':
    train_dir = 'DOWNLOAD_PATH/FOLDER/news-recommendation/data/original/train'
    val_dir = 'DOWNLOAD_PATH/FOLDER/news-recommendation/data/original/val'
    test_dir = 'DOWNLOAD_PATH/FOLDER/news-recommendation/data/original/test'
    ...
    #     f'/data/zhaoyy/news_data/data/glove/glove.840B.{config.word_embedding_dim}d.txt',
