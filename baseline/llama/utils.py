...
def pretrained_encode_bert(news_dataframe, repeat_times_title, repeat_times_abstract):
    bert_path = "DOWNLOAD_PATH/llms/bert-base-uncased"
...
def pretrained_encode_glove(news_dataframe, num_title, num_abstract):
    ...
    glove_path = "DOWNLOAD_PATH/llms/glove/glove.840B.300d.txt"
...
@timer
def pretrained_encode_llama(news_dataframe, repeat_times_title, repeat_times_abstract, llama_mode):

    # Load LLaMA model and tokenizer
    if model_name in NAML_like:
        last_token_length = 5
    else: 
        last_token_length = 10
    llama_path = {"ori": "DOWNLOAD_PATH/llms/Meta-Llama-3.1-8B-Instruct",
                  "dpo": "DOWNLOAD_PATH/llms/llama3_dpo_merged",
                  "sft": "DOWNLOAD_PATH/llms/llama3_sft_merged"}
    model_path = llama_path[llama_mode]
    ...
    def encode_text(texts, repeat_times, mode="title"):
        all_embeddings = []
        batch_size = 10  # Adjusted smaller batch size if necessary
        for i in range(0, len(texts), batch_size):
            batch_texts = texts[i:i + batch_size]
            ...
            with torch.no_grad():
                ...
                last_layer = hidden_states[-1][:, -last_token_length:, :]
                if last_layer.shape[1] < last_token_length:

                    pad_size = last_token_length - last_layer.shape[1]
                    ...
                    outputs = last_layer
                assert outputs.shape[1] == last_token_length
    ...
    if model_name in NAML_like:
        news_dataframe['title_encoded'] = encode_text(news_dataframe['title'], repeat_times_title)
        news_dataframe['abstract_encoded'] = encode_text(news_dataframe['abstract'], repeat_times_abstract, mode="abstract")
