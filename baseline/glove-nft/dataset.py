...
#     def __init__(self, behaviors_path, news_path, pretrained_mode):
...
#                 # Convert list data to tensors, keep strings intact
#                 if isinstance(self.news2dict[key1][key2], list):
#                     self.news2dict[key1][key2] = torch.tensor(self.news2dict[key1][key2], dtype=torch.long)
#         padding_all = {
#             'category': 0,
#             'subcategory': 0,
#             'title': '',  # Keeping title as an empty string for consistency in handling text
#             'abstract': '',  # Same for abstract
...
#             if isinstance(padding_all[key], list):
#                 padding_all[key] = torch.tensor(padding_all[key], dtype=torch.long)
...
class BaseDataset(Dataset):
    def __init__(self, behaviors_path, news_path, config):
        ...
        if config.pretrained_mode == 'bert':
            ...
            self.news_parsed_encoded = pretrained_encode_llama(self.news_parsed, repeat_times_title=config.num_words_title, repeat_times_abstract=config.num_words_abstract)
        ...
        padding_all = {
            'category': 0,
            'subcategory': 0,
            'title': [[0.0]* config.word_embedding_dim] * config.num_words_title,  # Keeping title as an empty string for consistency in handling text
            'abstract': [[0.0]* config.word_embedding_dim] * config.num_words_abstract,  # Same for abstract
            'title_entities': [0] * config.num_words_title,  # Assuming these are numerical lists
            'abstract_entities': [0] * config.num_words_abstract
        }
        for key in padding_all.keys():
